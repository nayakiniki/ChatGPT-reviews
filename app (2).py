...
if not st.session_state.csv_uploaded:
    ...
    # Morphing Text - static text with morph effect, centered and white
    st.markdown(
    ...
    st.markdown(
        "<div style='text-align:center;color:#ffffff;font-size:1.3rem;'>Upload your ChatGPT Reviews CSV:</div>", 
        unsafe_allow_html=True
    )
    ...
    if 'Review Date' in filtered_df.columns and len(filtered_df):
        try:
            ...
            fig, ax = plt.subplots(figsize=(12,6))
            ax.plot(rating_trend.index, rating_trend, label='Avg Rating', color="#ffffff", marker="o")
    ...
    st.markdown(
        ...
        "Made with ❤️ using Streamlit | <a href='https://github.com/nayakiniki/ChatGPT-reviews' style='color:#ffffff;'>GitHub Repo</a>"
